...
def get_filepaths(maxDate=None, maxSampleNum=None, includeRepeats=False):
    ...
    for folder in all_folders:  # For each subfolder
        og_folder = folder #save complete folder name
        if folder.startswith('Natsumi-'): #Workaround for Aurelie's new folder names
            folder = folder[8:]
...
def gateFullDataset(my_sample, save_images=True, *args, **kwargs):
    ...
    my_sample.update(ag.AGgate(PBMC, None,'FSC 488/10-A', 'SSC 488/10-A', "PBMC"), QC=True, scale='linear',xlim=[25000,180000], ylim=[0,200000])
    ...
    my_sample.update(ag.AGgate(CD34pos, CD45pos,'CD34 PE-Cy7-A', CD45, "CD34+"), QC=True, scale='bilog', T=200, xlim=[-1000, 100000], ylim=[1000, 150000])
    ...
    my_sample.update(ag.AGgate(linnegCD34pos, CD45pos,'CD34 PE-Cy7-A' , CD45,"linneg_cd34pos"), QC=True,  scale='bilog', T=200, xlim=[-1000, 100000], ylim=[1000, 150000])
...
if __name__ == '__main__':
    ...
    filepaths = get_filepaths(maxDate=date(2021,5,7), maxSampleNum=3020)  # Include only samples with number below maxSampleNum
